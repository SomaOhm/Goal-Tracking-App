...
export const FAB: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addGoal({ title, description, frequency, visibleToGroups: [] });
    setTitle('');
    setDescription('');
    setFrequency('daily');
    setOpen(false);
        ...
        <DialogContent className="rounded-3xl max-w-md mx-4 bg-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div>
              ...
              <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="What does this goal mean to you?" rows={3} className="rounded-2xl border-[#E0D5F0] focus:border-[#C8B3E0] bg-[#FFFBF7]" />
            </div>
            <div>
